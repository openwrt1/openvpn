...
function installQuestions() {
	...
	echo "Git 仓库地址：https://github.com/yhdxtn/OpenVpn"
	echo ""
	...
	echo "你可以选择优先使用 IPv4 或 IPv6。"
	echo ""

	until [[ $DETECT_IPV4 =~ (y|n) ]]; do
		read -rp "是否优先检测并使用 IPv4 地址? (如果服务器没有 IPv4, 请选择 'n') [y/n]: " -e -i y DETECT_IPV4
	done
...
verb 3" >>/etc/openvpn/server.conf
	...
	if [[ $OS == 'arch' || $OS == 'fedora' || $OS == 'centos' || $OS == 'oracle' ]]; then
		...
		sed -i '/[Service]/a DeviceAllow=/dev/net/tun rw' /etc/systemd/system/openvpn-server@.service
		...
		sed -i '/[Service]/a DeviceAllow=/dev/net/tun rw' /etc/systemd/system/openvpn\@.service
		# 另一个继续使用 /etc/openvpn/ 的解决方法
