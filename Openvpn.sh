...
function installOpenVPN() {
	...
	if [[ ! -e /etc/openvpn/server.conf && ! -e /etc/openvpn/server-ipv4.conf ]]; then
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			apk add --update openvpn easy-rsa iptables openssl wget ca-certificates curl bash
		fi
...
function removeOpenVPN() {
	...
	if [[ $REMOVE == 'y' ]]; then
		...
		# 强制清理残留的 tun 接口，这是确保干净卸载的关键
		for i in $(ip link | grep -o 'tun[0-9]*'); do
			echo "正在移除残留的接口: $i"
			ip link delete "$i"
		done
